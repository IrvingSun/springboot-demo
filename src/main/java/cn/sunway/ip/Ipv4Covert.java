...
public class Ipv4Covert {
    public static void main(String[] args) {
        ...
        long ip22 = ipToLong(ip);
        System.out.println("长整型ip ----> " + ip22);

        // step3: 对整型变量进行右位移处理，恢复IP字符串
        String ip3 = intToIp(ip2);
        System.out.println("字符串ip---->" + ip3);
        System.out.println("字符串ip_longToIp---->" + longToIp(ip22));
    ...
    public static String longToIp(long data){
        String ip = String.valueOf(data >>> 24) + "." +
                ((data >>> 16) & 0xFF) + "." +
                ((data >>> 8) & 0xFF) + "." +
                ((data) & 0xFF);
        return ip;
